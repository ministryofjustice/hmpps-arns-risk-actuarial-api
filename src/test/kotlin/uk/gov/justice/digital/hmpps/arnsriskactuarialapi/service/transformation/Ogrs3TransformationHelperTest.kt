...
class Ogrs3TransformationHelperTest {
    ...
    inner class AgeGenderParameterTest {
        @Test
        fun `getOffenderAgeGroup should return age group`() {
            val actual = getOffenderAgeGroup(10)
        ...
        fun `getOffenderAgeGroup should throw error when under ten`() {
            val exception = assertThrows(IllegalArgumentException::class.java) {
                getOffenderAgeGroup(9)
            }
            assert(exception.message!!.contains("Age must be age 10 or more"))
        }

        @Test
        fun `getOffenderAgeGroup should throw error when negative value`() {
            val exception = assertThrows(IllegalArgumentException::class.java) {
                getOffenderAgeGroup(-1)
            }
