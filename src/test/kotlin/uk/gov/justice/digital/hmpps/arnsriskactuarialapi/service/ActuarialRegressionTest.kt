...
class ActuarialRegressionTest {
  ...
  fun `requests from CSV file`(
    ...
    val request = ActuarialScoreRequest().copy(
      dateOfBirth = dob.toDateTime(),
      gender = gender.toGenderBoolean(),
      assessmentDate = "2015/10/03",
      dateOfCurrentConviction = last_sanction_date.toDateTime(),
      currentOffenceCode = offence_code.toIntOrNull(),
      totalNumberOfSanctionsForAllOffences = total_sanctions_count.toIntOrNull(),
      ageAtFirstSanction = ageatfirst.toIntOrNull(),
      dateAtStartOfFollowupUserInput = last_sanction_date.toDateTime(),
      dateAtStartOfFollowupCalculated = community_date.toDateTime(),
...
fun String?.toRelationshipScore(): RelationshipLevel? = when (this?.toIntOrNull()) {
  1 -> ProblemLevel.IN_RELATIONSHIP_LIVING_TOGETHER
  2 -> ProblemLevel.IN_RELATIONSHIP_NOT_LIVING_TOGETHER
  3 -> ProblemLevel.NOT_IN_RELATIONSHIP
  else -> null
}
...
private fun String?.toYesNoBoolean(): Boolean? = when {
  this?.contains("Y") == true -> true
  this?.contains("N") == true -> false
  else -> null
}
...
private fun String?.toGenderBoolean(): Boolean? = when {
