...
class SNSVTransformationHelper {
  companion object {
    ...
    const val STATIC_FEMALE_WEIGHT = -16.6220270089011
    const val DYNAMIC_FEMALE_WEIGHT = -16.6927292697847
    val STATIC_NUMBER_OF_SANCTIONS_WEIGHTS = doubleArrayOf(-2.09447596484765, -1.67613460779912)
    val DYNAMIC_NUMBER_OF_SANCTIONS_WEIGHTS = doubleArrayOf(-1.89458617745666, -1.51763151836726)
    val STATIC_TOTAL_SANCTIONS_WEIGHTS = -0.0147495874606046
    val DYNAMIC_TOTAL_SANCTIONS_WEIGHTS = -0.0182592921752245
    ...
    val STATIC_THREE_PLUS_SANCTIONS_WEIGHT =
      mapOf(Gender.MALE to 0.769213898314811, Gender.FEMALE to 0.793186572461819)
    ...
    fun getNumberOfSanctionWeight(totalNumberOfSanctionsForAllOffences: Int, isSNSVDynamic: Boolean): Double = // OGRS3 contribution
      when (isSNSVDynamic) {
        ...
        when {
          ...
          totalNumberOfSanctionsForAllOffences > 2 -> 0.0
          else -> throw IllegalArgumentException("Invalid total number of sanctions value: $totalNumberOfSanctionsForAllOffences")
        }
      }

    fun getTotalSanctionWeight(totalNumberOfSanctionsForAllOffences: Int, isSNSVDynamic: Boolean): Double = // OGRS3 contribution
      when (isSNSVDynamic) {
        true -> DYNAMIC_TOTAL_SANCTIONS_WEIGHTS
        false -> STATIC_TOTAL_SANCTIONS_WEIGHTS
      }.let { weights ->
        when {
          totalNumberOfSanctionsForAllOffences > 0 -> totalNumberOfSanctionsForAllOffences * weights
          else -> throw IllegalArgumentException("Invalid total number of sanctions value: $totalNumberOfSanctionsForAllOffences")
        }
      }
    ...
    fun getYearsBetweenFirstAndSecondSanctionWeight(
      ...
      if (totalNumberOfSanctionsForAllOffences != 2) return 0.0
    ...
    fun getViolentHistoryWeight(
      ...
      if (isSNSVDynamic) {
        Pair(DYNAMIC_GENDERED_VIOLENT_SANCTIONS_WEIGHTS, DYNAMIC_ONCE_VIOLENT_SANCTIONS_WEIGHTS)
      } else {
        Pair(STATIC_GENDERED_VIOLENT_SANCTIONS_WEIGHTS, STATIC_ONCE_VIOLENT_SANCTIONS_WEIGHTS)
      }
      ...
      when {
        ...
        totalNumberOfViolentSanctions > 1 -> 0.0
    ...
    fun getViolentSanctionsWeight(
      ...
      if (isSNSVDynamic) DYNAMIC_TOTAL_VIOLENT_SANCTIONS_WEIGHTS else STATIC_TOTAL_VIOLENT_SANCTIONS_WEIGHTS
      )
      .let { nonGenderedWeights ->
        when {
          totalNumberOfViolentSanctions == 0 -> 0.0
          totalNumberOfViolentSanctions > 0 -> nonGenderedWeights * totalNumberOfViolentSanctions
          else -> throw IllegalArgumentException("Invalid total number of violent sanctions value: $totalNumberOfViolentSanctions")
        }
      }
    ...
    fun getViolenceRateWeight(
      ...
      if (totalNumberOfViolentSanctions == 0) return 0.0
