...
import kotlin.math.ln
  ...
  fun getAgeAt(stage: String, dateOfBirth: LocalDate, dateAtStage: LocalDate, lowest: Int): Int =
    Period.between(dateOfBirth, dateAtStage).years
      .apply {
  ...
  fun getNumberOfSanctionsWeight(totalNumberOfSanctionsForAllOffences: Int, isSNSVDynamic: Boolean): BigDecimal {
    val staticNumberOfSanctionsCoefficients = arrayOf(
      SeriousViolentReoffendingPredictorStatic.FIRST_SANCTION.coefficient,
      SeriousViolentReoffendingPredictorStatic.SECOND_SANCTION.coefficient,
    )

    val dynamocNumberOfSanctionsCoefficients = arrayOf(
      SeriousViolentReoffendingPredictorDynamic.FIRST_SANCTION.coefficient,
      SeriousViolentReoffendingPredictorDynamic.SECOND_SANCTION.coefficient,
    )

    return when (isSNSVDynamic) {
      true -> dynamocNumberOfSanctionsCoefficients
      false -> staticNumberOfSanctionsCoefficients
    }.let { weights ->
      when {
        totalNumberOfSanctionsForAllOffences == 1 -> weights[0]
        totalNumberOfSanctionsForAllOffences == 2 -> weights[1]
        totalNumberOfSanctionsForAllOffences > 2 -> BigDecimal.ZERO
        else -> throw IllegalArgumentException("Invalid total number of sanctions value: $totalNumberOfSanctionsForAllOffences")
      }
    }
  }

  fun getTotalSanctionWeight(totalNumberOfSanctionsForAllOffences: Int, isSNSVDynamic: Boolean): BigDecimal =
    when (isSNSVDynamic) {
  ...
  fun getViolentHistoryWeight(
    ...
    if (isSNSVDynamic) {
      Pair(
        ...
        SeriousViolentReoffendingPredictorDynamic.ONCE_VIOLENT.coefficient,
      )
    } else {
      Pair(
        mapOf(
        ...
        SeriousViolentReoffendingPredictorStatic.ONCE_VIOLENT.coefficient,
      )
    }
    ).let { (genderedWeights, onceViolentWeights) ->
      when {
        totalNumberOfViolentSanctions == 0 -> genderedWeights[gender]!!
        totalNumberOfViolentSanctions == 1 -> onceViolentWeights
        totalNumberOfViolentSanctions > 1 -> BigDecimal.ZERO
  ...
  fun didOffenceInvolveCarryingOrUsingWeaponWeight(carryingOrUsingWeapon: Boolean): BigDecimal =
    if (carryingOrUsingWeapon) SeriousViolentReoffendingPredictorDynamic.CARRY_OR_USE_WEAPON.coefficient else BigDecimal.ZERO

  fun suitabilityOfAccommodationWeight(suitabilityOfAccommodation: ProblemLevel): BigDecimal =
    SeriousViolentReoffendingPredictorDynamic.ACCOMMODATION_SUITABILITY.coefficient * suitabilityOfAccommodation.score.toBigDecimal()

  fun isUnemployedWeight(isUnemployed: Boolean): BigDecimal =
    if (isUnemployed) SeriousViolentReoffendingPredictorDynamic.UNEMPLOYED.coefficient else BigDecimal.ZERO

  fun chronicDrinkingProblemsWeight(currentAlcoholUseProblems: ProblemLevel): BigDecimal =
    currentAlcoholUseProblems.score.toBigDecimal() * SeriousViolentReoffendingPredictorDynamic.CHRONIC_DRINKING.coefficient

  fun temperControlWeight(temperProblems: ProblemLevel): BigDecimal =
    temperProblems.score.toBigDecimal() * SeriousViolentReoffendingPredictorDynamic.TEMPER.coefficient

  fun proCriminalAttitudesWeight(proCriminalAttitudes: ProblemLevel): BigDecimal =
    proCriminalAttitudes.score.toBigDecimal() * SeriousViolentReoffendingPredictorDynamic.PRO_CRIMINAL_ATTITUDE.coefficient

  fun previousConvictionsWeight(previousConvictions: List<PreviousConvictions>): BigDecimal =
    previousConvictions.sumOf { conviction -> conviction.snsvDynamicWeight }
